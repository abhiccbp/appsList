...
class AppStore extends Component {
  ...
  getFilteredResults = abc => {
    const {activeTabId} = this.state
    const filteredResults = abc.filter(each => each.category === activeTabId)
  ...
  render() {
    ...
    const filteredResults = this.getFilteredResults(searchResults)

    return (
      <div className="container1">
        <div className="container2">
          <h1>App Store</h1>
          <input onChange={this.onClickSearch} type="search" />
          <img
